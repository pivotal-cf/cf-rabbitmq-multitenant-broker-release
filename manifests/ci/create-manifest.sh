...
bosh interpolate --var deployment-name=cf-rabbitmq-multitenant-broker-release-ci \
	...
	--var loggregator-agent-tls-cert="((/bosh-${ENVIRONMENT}/cf/loggregator_ca.certificate))" \
	--var loggregator-agent-tls-key="((/bosh-${ENVIRONMENT}/cf/loggregator_ca.private_key))" \
